...
const createBorrowBook = `-- name: CreateBorrowBook :one
...
func (q *Queries) CreateBorrowBook(ctx context.Context, arg CreateBorrowBookParams) (BorrowBook, error) {
	row := q.db.QueryRowContext(ctx, createBorrowBook, arg.StudentID, arg.DueDate)
	var i BorrowBook
	err := row.Scan(
		&i.ID,
		&i.StudentID,
...
const returnBorrowBook = `-- name: ReturnBorrowBook :execrows
UPDATE "Borrow_Book"
SET is_return = true, updated_at = now(), return_date = now()
WHERE id = $1 AND is_return = false
`

func (q *Queries) ReturnBorrowBook(ctx context.Context, id int32) (int64, error) {
	result, err := q.db.ExecContext(ctx, returnBorrowBook, id)
	if err != nil {
		return 0, err
	}
